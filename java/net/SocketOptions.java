...
public interface SocketOptions {
     ...
     * @see #getOption(int)
     *
     * 设置socket选项，value必须为选项的指定的类型
     * 所有选项都可以使用 #Boolean.FALSE禁用
     *
     */
    public void
     ...
     *
     * 获取套接字的本地地址绑定（此选项不能仅“设置”，因为套接字在创建时绑定，
     * 因此本地绑定的地址不能被更改）。
     * 套接字的默认本地地址为INADDR_ANY，
     * 表示多宿主主机上的任何本地地址。
     * 多宿主主机可以使用此选项来接受仅与其一个地址的连接（在ServerSocket或DatagramSocket的情况下），
     * 或者指定其对等体（对于Socket或DatagramSocket）的返回地址。 此选项的参数是InetAddress。
     */
     ...
     *
     * 为套接字设置SO_REUSEADDR。 这仅用于MulticastSockets
     ...
     * @since 1.4
     *
     * 为套接字设置SO_BROADCAST。 此选项启用和禁用进程发送广播消息的功能。
     * 只支持数据报套接字，仅在支持广播消息（例如以太网，令牌环等）概念的网络上，
     */
     ...
     *
     * 设置发送组播数据包的出接口。
    适用于具有多个网络接口的主机，其中应用程序希望使用除系统默认值。 获取/返回InetAddress。
     */
     ...
     * @since 1.4
     *
     * 引入此选项，以便IP_MULTICAST_IF的行为将保持与以前相同，
     * 而此新选项可以支持使用IPv4和IPv6地址设置输出接口。
       注意：确保没有与此冲突
     */
